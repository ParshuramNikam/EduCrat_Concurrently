...
const UserProfileDetails = ({ userData, paramsUser }) => {
  ...
  useEffect(() => {
    if (paramsUser) {
      axios.get(backendURL + '/api/user/' + paramsUser._id).then(res => {
        // console.log(res.data.user);
        setUserDetails(res.data.user);
        if (res.data.user.role === 0) {
          setAction("edit");
        } else {
          setAction("editTeacherProfile");
        }
      ...
      axios
        ...
        .then(async (res) => {
          ...
          res.data.user.feedbackInfo.map((feedback) => {
            console.log( feedback?.tutorId === paramsUser._id);
            if (
              feedback?.tutorId === paramsUser._id &&
          ...
          <div className="name_wrapper">
            ...
            <div className="star_rate">
              ...
              <span className="actual_rating">
                ...
                ({ratingCount}) */}
                <span className="rating">{Number.parseInt(ratingCount) > 0 ? Number.parseFloat(Number.parseInt(overallRating)/ratingCount) : 0}</span>
                <span className="rating-no">({ratingCount} Ratings)</span>
